...
## PrimitiveValueComparer.Compare(float, decimal) Method

Compares a Single to a Decimal

```csharp
public static int Compare(float x, decimal y);
```
#### Parameters

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(float,decimal).x'></a>

`x` [System.Single](https://docs.microsoft.com/en-us/dotnet/api/System.Single 'System.Single')

The first item to compare

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(float,decimal).y'></a>
