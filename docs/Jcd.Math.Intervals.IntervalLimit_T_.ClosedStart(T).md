...
## IntervalLimit<T>.ClosedStart(T) Method
...
```csharp
public static Jcd.Math.Intervals.IntervalLimit<T> ClosedStart(T limit);
```
#### Parameters

<a name='Jcd.Math.Intervals.IntervalLimit_T_.ClosedStart(T).limit'></a>
