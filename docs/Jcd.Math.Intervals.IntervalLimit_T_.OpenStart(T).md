...
## IntervalLimit<T>.OpenStart(T) Method
...
```csharp
public static Jcd.Math.Intervals.IntervalLimit<T> OpenStart(T limit);
```
#### Parameters

<a name='Jcd.Math.Intervals.IntervalLimit_T_.OpenStart(T).limit'></a>
