...
## IntervalLimit<T>.ClosedEnd(T) Method
...
```csharp
public static Jcd.Math.Intervals.IntervalLimit<T> ClosedEnd(T limit);
```
#### Parameters

<a name='Jcd.Math.Intervals.IntervalLimit_T_.ClosedEnd(T).limit'></a>
