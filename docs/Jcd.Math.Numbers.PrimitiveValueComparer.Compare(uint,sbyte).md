...
## PrimitiveValueComparer.Compare(uint, sbyte) Method

Compares a UInt32 to a SByte

```csharp
public static int Compare(uint x, sbyte y);
```
#### Parameters

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(uint,sbyte).x'></a>

`x` [System.UInt32](https://docs.microsoft.com/en-us/dotnet/api/System.UInt32 'System.UInt32')

The first item to compare

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(uint,sbyte).y'></a>
