...
## PrimitiveValueComparer.Equals(sbyte, uint) Method

Checks a SByte and a UInt32 for equality

```csharp
public static bool Equals(sbyte x, uint y);
```
#### Parameters

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Equals(sbyte,uint).x'></a>

`x` [System.SByte](https://docs.microsoft.com/en-us/dotnet/api/System.SByte 'System.SByte')

The first item to compare

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Equals(sbyte,uint).y'></a>
