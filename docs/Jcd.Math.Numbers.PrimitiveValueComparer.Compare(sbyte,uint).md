...
## PrimitiveValueComparer.Compare(sbyte, uint) Method

Compares a SByte to a UInt32

```csharp
public static int Compare(sbyte x, uint y);
```
#### Parameters

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(sbyte,uint).x'></a>

`x` [System.SByte](https://docs.microsoft.com/en-us/dotnet/api/System.SByte 'System.SByte')

The first item to compare

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(sbyte,uint).y'></a>
