...
## PrimitiveValueComparer.Compare(ulong, uint) Method

Compares a UInt64 to a UInt32

```csharp
public static int Compare(ulong x, uint y);
```
#### Parameters

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(ulong,uint).x'></a>

`x` [System.UInt64](https://docs.microsoft.com/en-us/dotnet/api/System.UInt64 'System.UInt64')

The first item to compare

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(ulong,uint).y'></a>
