...
## PrimitiveValueComparer.Compare(float, uint) Method

Compares a Single to a UInt32

```csharp
public static int Compare(float x, uint y);
```
#### Parameters

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(float,uint).x'></a>

`x` [System.Single](https://docs.microsoft.com/en-us/dotnet/api/System.Single 'System.Single')

The first item to compare

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Compare(float,uint).y'></a>
