...
## PrimitiveValueComparer.Equals(uint, float) Method

Checks a UInt32 and a Single for equality

```csharp
public static bool Equals(uint x, float y);
```
#### Parameters

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Equals(uint,float).x'></a>

`x` [System.UInt32](https://docs.microsoft.com/en-us/dotnet/api/System.UInt32 'System.UInt32')

The first item to compare

<a name='Jcd.Math.Numbers.PrimitiveValueComparer.Equals(uint,float).y'></a>
