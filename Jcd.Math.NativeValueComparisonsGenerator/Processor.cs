...
public static class Processor
{
    ...
    public static string Generate(Upcast upcast)
    {
        if (upcast.First == typeof(bool))
            return DowncastToBool(upcast.First, upcast.Second);

        if (upcast.First == upcast.Second)
            return string.Empty;
    
        if (upcast.First == upcast.Target)
            return UpcastSecond(upcast.First, upcast.Second, upcast.Target);
        
        return UpcastBoth(upcast.First, upcast.Second, upcast.Target);
    }
}